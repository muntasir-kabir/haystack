...
| Source-line timeline + filter lanes (record-start overview counts resolved through the document rank index; physical-line match counts; exact viewport re-resolution while zooming/panning; source-order markers, navigation, minimap, trim, and persisted zoom; timestamps remain annotations/query metadata and are never sorted into GUI coordinates; append rebuilds exact bucket counts without reprojecting old centers; worker construction keeps the UI responsive) | `src/core/timeline.rs`, `src/ui/timeline/`, `src/ui/app/model.rs` |
