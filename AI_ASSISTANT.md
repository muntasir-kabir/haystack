...
2. Any **Changes** — add a 1-2 sentence entry at the top of `changes.md` describing what changed.
3. **Tests** is must for code level change.
  - No need to run fmt, check, test if code is not changed.
  — For small code change run "Focused Verification": run only relevent cargo format, check, tests - Only for potentially affected files/modules.
  - For only Big/Multi feature/Domain feature changes run "Detail Verification": need to run full cargo `fmt`, `check`, `test` with elevated permission for all target.
...
- Single binary: `haystack` (GUI) or `haystack --mcp` (stdio MCP server for standalone and explicitly GUI-attached work). `--mcp-gui` is a deprecated compatibility alias only.
...
- `src/ui/` — egui GUI modules organized by feature:
  - `app/` — application state and main UI loop: `model.rs` separates file-level `LogTab` state from per-viewport `LogViewState`, `tab_model.rs` contains their operations, `view.rs` is the main shell, and the focused `filters_dropdown.rs` / `templates.rs` views handle saved filters and template browsing
  ...
  - `record_format/` — log-format template editor, bounded preview, and preset/apply actions
...
- `cargo run --release --no-default-features --example bench_record -- FILE default|detailed [FILTER ...]` — measure the production parse/timeline path for an explicitly selected record layout (use deterministic `gen_record_logs` inputs)
