...
- Use the External Window action beside any Log, Pinned, or Templates tab to detach it. A detached native window is a free-form dock: Log, Pinned, and Templates tabs can share it and may be arranged in any direction. Dragging starts after a small pointer movement and shows a grabbing cursor and compact tab ghost. Tab bars show an exact insertion line; detached panes preview center joins and left/right/top/bottom splits. The main window exposes only the legal upper Log or lower utility target. Moving outside every legal target shows a lightweight window rectangle; release there creates a native detached window at that position without rendering live pane contents during the preview. The left-arrow action returns the whole detached layout to the main window. A detached window remains open when its original tab moves elsewhere and closes only after its final tab is transferred; closing the native window also returns its tabs to their legal main-window homes.
- Saved investigations retain detached window groupings, split layouts, positions, and sizes. Older compatible investigations that only recorded detached tabs continue to reopen each recorded tab in its own window.
